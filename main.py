...
# Groq Client (uses user's key stored in session)
...
if "resume_chat_history" not in st.session_state:
    st.session_state.resume_chat_history = [
        {
            "role": "assistant",
            "content": "Hi! I'm the Resume Analyzer Assistant. Ask me anything about ATS score, bias detection, supported PDF types, or how the analysis works."
        }
    ]

# ----------------- SYSTEM PROMPT -----------------
RESUME_ASSISTANT_SYSTEM_PROMPT = """
You are the official Hirelyzer Resume Analyzer AI Assistant.

You ONLY answer questions related to:
- Resume Analysis
- ATS scoring logic
- detect_bias() algorithm
- masculine/feminine gender-coded words
- bias score normalization formula
- rewrite_and_highlight() logic
- PDF → text extraction (PyMuPDF)
- OCR fallback using EasyOCR
- how ATS uses LLaMA scoring + keyword match
- section weight sliders
- dashboard metrics and charts
- supported file formats

ABSOLUTELY DO NOT answer anything unrelated to resume analysis.
If asked an unrelated question, reply:
“I'm here only to assist with Resume Analyzer questions.”
...
if st.session_state.resume_chat_open:
    ...
    st.markdown("<h4>🤖 Resume Analyzer Assistant</h4>", unsafe_allow_html=True)
    ...
    # ---------- USER INPUT ----------
    user_msg = st.text_input("Type your question:", key="resume_chat_input")
    ...
    if st.button("Send", key="send_resume_chat"):
        if user_msg.strip():
            ...
            try:
                ...
                bot_reply = f"Could not reach Groq LLaMA: {str(e)}"
