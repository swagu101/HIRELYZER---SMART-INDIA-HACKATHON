...
with tab4:
    ...
    page = st.radio(
        ...
        options=["Courses by Role", "Resume Videos", "Interview Videos",  "AI Interview Coach 🤖", "Leaderboard Dashboard 🏅"],
        horizontal=True,
        key="page_selection",
...
"""
    ...
    # Section 5: NEW Leaderboard Dashboard 🏅
    elif page == "Leaderboard Dashboard 🏅":
        import sqlite3
        import pandas as pd
        from streamlit_autorefresh import st_autorefresh

        st.subheader("🏅 Leaderboard Dashboard")
        st.markdown("Track global performance, analyze domain trends, and see how you stack up against other candidates!")

        try:
            st_autorefresh(interval=30000, key="leaderboard_refresh")
        except:
            pass
        ...
        try:
            ...
            if total_interviews == 0:
                st.info("🚀 No interview data yet. Complete an interview to see your results on the leaderboard!")
                conn.close()
            else:
                st.markdown(f"""
                <div style="background: linear-gradient(135deg, rgba(0, 195, 255, 0.12) 0%, rgba(0, 195, 255, 0.06) 100%);
                            backdrop-filter: blur(15px);
                            -webkit-backdrop-filter: blur(15px);
                            border: 1px solid rgba(0, 195, 255, 0.25);
                            border-radius: 20px;
                            padding: 20px;
                            margin: 20px 0;
                            box-shadow: 0 8px 32px rgba(0, 195, 255, 0.15), inset 0 1px 0 rgba(255, 255, 255, 0.08);">
                    <h3 style="color: #00c3ff; text-align: center; margin: 0; font-size: 22px;">
                        📊 Total Interviews Completed: {total_interviews}
                    </h3>
                </div>
                """, unsafe_allow_html=True)
                ...
                with col1:
                    timeframe = st.selectbox(
                        "📅 Timeframe",
                        options=["All Time", "Weekly", "Monthly"],
                        key="timeframe_filter"
                    )

                with col2:
                    domain_list = cursor.execute("SELECT DISTINCT domain FROM interview_results").fetchall()
                    domain_options = ["All Domains"] + [d[0] for d in domain_list if d[0]]
                    selected_domain = st.selectbox(
                        "🌐 Domain Filter",
                ...
                timeframe_query = ""
                if timeframe == "Weekly":
                    timeframe_query = "AND completed_on >= date('now', '-7 day')"
                elif timeframe == "Monthly":
                    timeframe_query = "AND completed_on >= date('now', '-30 day')"

                domain_query = ""
                if selected_domain != "All Domains":
                    domain_query = f"AND domain = '{selected_domain}'"

                st.markdown("---")
                st.markdown('<h2 style="color: #00c3ff; text-align: center;">🌍 Global Performance Overview</h2>', unsafe_allow_html=True)

                query = f"""
                SELECT username, role, domain, avg_score, completed_on, total_questions
                FROM interview_results
                WHERE 1=1 {timeframe_query} {domain_query}
                ...
                if not top_users.empty:
                    ...
                    for idx in range(len(top_users)):
                        if idx == 0:
                            ...
                            colors.append('#00c3ff')

                    fig_leaderboard = go.Figure()
                    fig_leaderboard.add_trace(go.Bar(
                        y=top_users['username'],
                        x=top_users['avg_score'],
                        orientation='h',
                        marker=dict(
                            color=colors,
                            line=dict(color='rgba(255, 255, 255, 0.3)', width=1)
                        ),
                        text=top_users['avg_score'].round(2),
                        textposition='auto',
                        hovertemplate='<b>%{y}</b><br>Score: %{x:.2f}/10<br><extra></extra>'
                    ))

                    fig_leaderboard.update_layout(
                        title=dict(
                            text="Top Performers by Average Score",
                            x=0.5,
                            font=dict(color='#00c3ff', size=18)
                        ),
                        xaxis=dict(
                            title="Average Score",
                            range=[0, 10],
                    ...
                    display_df['Rank'] = ['🥇', '🥈', '🥉'] + ['🏅'] * (len(display_df) - 3)
                    display_df = display_df[['Rank', 'username', 'role', 'domain', 'avg_score', 'total_questions', 'completed_on']]
                    display_df.columns = ['Rank', 'Username', 'Role', 'Domain', 'Avg Score', 'Questions', 'Completed On']
                ...
                st.markdown("---")
                st.markdown('<h2 style="color: #00c3ff; text-align: center;">📊 Domain Insights</h2>', unsafe_allow_html=True)

                domain_query_filter = ""
                if selected_domain != "All Domains":
                    domain_query_filter = f"WHERE domain = '{selected_domain}'"

                domain_stats = pd.read_sql_query(f"""
                    SELECT domain,
                           AVG(avg_score) AS avg_domain_score,
                ...
                if not domain_stats.empty:
                    ...
                    with col1:
                        top_domain = domain_stats.loc[domain_stats['avg_domain_score'].idxmax()]
                        st.markdown(f"""
                        <div style="background: rgba(0, 195, 255, 0.07);
                                    backdrop-filter: blur(15px);
                                    border-radius: 20px;
                                    border: 1px solid rgba(255, 255, 255, 0.15);
                                    padding: 20px;
                            ...
                            <h3 style="color: #FFD700; margin: 0 0 10px 0;">🥇 Top Domain</h3>
                            <p style="color: white; font-size: 18px; margin: 5px 0;">{top_domain['domain']}</p>
                            <p style="color: #00c3ff; font-size: 24px; font-weight: bold; margin: 5px 0;">{top_domain['avg_domain_score']:.2f}/10</p>
                        </div>
                        """, unsafe_allow_html=True)

                    with col2:
                        low_domain = domain_stats.loc[domain_stats['avg_domain_score'].idxmin()]
                        st.markdown(f"""
                        <div style="background: rgba(0, 195, 255, 0.07);
                                    backdrop-filter: blur(15px);
                                    border-radius: 20px;
                                    ...
                                    text-align: center;
                                    box-shadow: 0 4px 30px rgba(0, 195, 255, 0.1);">
                            <h3 style="color: #87CEEB; margin: 0 0 10px 0;">🧊 Lowest Domain</h3>
                            <p style="color: white; font-size: 18px; margin: 5px 0;">{low_domain['domain']}</p>
                            <p style="color: #00c3ff; font-size: 24px; font-weight: bold; margin: 5px 0;">{low_domain['avg_domain_score']:.2f}/10</p>
                        </div>
                        """, unsafe_allow_html=True)

                    st.markdown("### 📊 Domain Performance Comparison")

                    col1, col2 = st.columns(2)

                    with col1:
                        fig_domain_bar = go.Figure()
                        fig_domain_bar.add_trace(go.Bar(
                            x=domain_stats['domain'],
                            y=domain_stats['avg_domain_score'],
                            marker=dict(
                                color='#00c3ff',
                                line=dict(color='rgba(255, 255, 255, 0.3)', width=1)
                            ),
                            text=domain_stats['avg_domain_score'].round(2),
                            textposition='auto',
                            hovertemplate='<b>%{x}</b><br>Avg Score: %{y:.2f}/10<br><extra></extra>'
                        ))

                        fig_domain_bar.update_layout(
                            title=dict(
                                text="Average Score by Domain",
                                x=0.5,
                                font=dict(color='#00c3ff', size=16)
                            ),
                            xaxis=dict(
                    ...
                    with col2:
                        fig_domain_pie = go.Figure()
                        fig_domain_pie.add_trace(go.Pie(
                            labels=domain_stats['domain'],
                            values=domain_stats['avg_domain_score'],
                            hole=0.4,
                            marker=dict(
                                colors=['#00c3ff', '#0099cc', '#007acc', '#0066cc', '#0052a3', '#003d7a'],
                                line=dict(color='rgba(255, 255, 255, 0.3)', width=2)
                            ),
                            textinfo='label+percent',
                            hovertemplate='<b>%{label}</b><br>Score: %{value:.2f}/10<br><extra></extra>'
                        ))

                        fig_domain_pie.update_layout(
                            title=dict(
                                text="Domain Performance Share",
                                x=0.5,
                                font=dict(color='#00c3ff', size=16)
                            ),
                            paper_bgcolor='rgba(0, 0, 0, 0)',
                        ...
                        st.plotly_chart(fig_domain_pie, use_container_width=True)

                    st.markdown("### 🧩 Domain Comparison Table")

                    display_domain_df = domain_stats.copy()
                    display_domain_df.columns = ['Domain', 'Avg Score', 'Max Score', 'Min Score', 'Total Users']
                    display_domain_df['Avg Score'] = display_domain_df['Avg Score'].round(2)
                    display_domain_df['Max Score'] = display_domain_df['Max Score'].round(2)
                    display_domain_df['Min Score'] = display_domain_df['Min Score'].round(2)
                ...
                st.markdown("---")
                st.markdown('<h2 style="color: #00c3ff; text-align: center;">⏱️ Recent Trends</h2>', unsafe_allow_html=True)

                trend_query = f"""
                SELECT DATE(completed_on) as date, AVG(avg_score) as avg_score
                FROM interview_results
                WHERE 1=1 {timeframe_query} {domain_query}
                ...
                if not trend_data.empty:
                    ...
                    fig_trend.add_trace(go.Scatter(
                        ...
                        marker=dict(size=8, color='#00c3ff', line=dict(color='white', width=2)),
                        fill='tonexty',
                        fillcolor='rgba(0, 195, 255, 0.2)',
                        hovertemplate='<b>Date: %{x}</b><br>Avg Score: %{y:.2f}/10<br><extra></extra>'
                    ))
                    ...
                    st.info("Not enough data to show trends for selected timeframe.")

                st.markdown("---")
                st.markdown('<h2 style="color: #00c3ff; text-align: center;">⚖️ Score Distribution</h2>', unsafe_allow_html=True)
                ...
                score_data = pd.read_sql_query(score_query, conn)
                ...
                if not score_data.empty:
                    ...
                    fig_hist.add_trace(go.Histogram(
                        ...
                        hovertemplate='Score Range: %{x}<br>Count: %{y}<br><extra></extra>'
                    ))

                    fig_hist.update_layout(
                        title=dict(
                            text="Score Distribution Across All Interviews",
                            x=0.5,
                            font=dict(color='#00c3ff', size=18)
                        ),
                        xaxis=dict(
                ...
                st.markdown("---")
                st.markdown('<h2 style="color: #00c3ff; text-align: center;">🥇 Badge Summary</h2>', unsafe_allow_html=True)

                badge_query = f"""
                SELECT
                    CASE
                        WHEN avg_score BETWEEN 0 AND 2.5 THEN '💪 Keep Practicing'
                        WHEN avg_score BETWEEN 2.6 AND 3.5 THEN '👍 Good Performer'
                        WHEN avg_score BETWEEN 3.6 AND 4.5 THEN '🌟 Star Performer'
                        WHEN avg_score BETWEEN 4.6 AND 5.0 THEN '🏆 Interview Ready'
                        ELSE 'Unranked'
                    END as badge,
                    COUNT(*) as count
                FROM interview_results
                ...
                if not badge_data.empty:
                    ...
                    with col1:
                        fig_badge_bar = go.Figure()
                        fig_badge_bar.add_trace(go.Bar(
                            y=badge_data['badge'],
                            x=badge_data['count'],
                            orientation='h',
                            marker=dict(
                                color=['#FFD700', '#00c3ff', '#32CD32', '#FF6347'],
                                line=dict(color='rgba(255, 255, 255, 0.3)', width=1)
                            ),
                            text=badge_data['count'],
                            textposition='auto',
                            hovertemplate='<b>%{y}</b><br>Count: %{x}<br><extra></extra>'
                        ))

                        fig_badge_bar.update_layout(
                            title=dict(
                        ...
                        st.plotly_chart(fig_badge_bar, use_container_width=True)

                    with col2:
                        fig_badge_donut = go.Figure()
                        fig_badge_donut.add_trace(go.Pie(
                            labels=badge_data['badge'],
                            values=badge_data['count'],
                            hole=0.5,
                            marker=dict(
                                colors=['#FFD700', '#00c3ff', '#32CD32', '#FF6347'],
                                line=dict(color='rgba(255, 255, 255, 0.3)', width=2)
                            ),
                            textinfo='label+percent',
                        ...
                        st.plotly_chart(fig_badge_donut, use_container_width=True)
